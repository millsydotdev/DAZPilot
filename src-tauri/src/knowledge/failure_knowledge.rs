...
impl FailureKnowledgeBase {
    ...
    pub fn record_failure(&self, context: FailureContext, action: &str, error: &str, resolution: Option<String>) {
    ...
    pub fn get_related_failures(&self, context: &FailureContext, limit: usize) -> Vec<FailureCase> {
        ...
        let mut scored: Vec<(f32, FailureCase)> = failures.values()
            .map(|f| {
    ...
    pub fn get_warnings_for_action(&self, context: &FailureContext, _action: &str) -> Vec<String> {
        ...
        for (error, count) in error_counts {
            if count >= 2 { // Only warn if we've seen this error multiple times
    ...
    pub fn get_suggested_alternatives(&self, _context: &FailureContext, _action: &str) -> Vec<String> {
    ...
    fn extract_failure_pattern(&self, failure: &FailureCase) -> String {
        ...
        if let Some(ref figure) = failure.context.scene_state.as_ref().and_then(|s| s.active_figure.as_ref()) {
    ...
    fn extract_success_pattern(&self, context: &FailureContext, action: &str) -> String {
        ...
        // Add context elements
        if let Some(ref figure) = context.scene_state.as_ref().and_then(|s| s.active_figure.as_ref()) {
            parts.push(format!("figure:{}", figure));
        }
    ...
    fn calculate_context_relevance(&self, ctx1: &FailureContext, ctx2: &FailureContext) -> f32 {
        ...
        let figure1 = ctx1.scene_state.as_ref().and_then(|s| s.active_figure.as_ref());
        let figure2 = ctx2.scene_state.as_ref().and_then(|s| s.active_figure.as_ref());
