...
impl AssetKnowledgeBase {
    ...
    pub fn get_semantic_asset(&self, asset_info: &AssetInfo) -> SemanticAsset {
        ...
        // Cache it
        self.semantic_assets.lock().unwrap().insert(
    ...
    fn generate_semantic_understanding(&self, asset_info: &AssetInfo) -> SemanticAsset {
        ...
        match asset_info.category.as_str() {
            ...
            "clothing" => {
                ...
                let compatible_figures: Vec<String> = asset_info.compatibility_base
                    .iter()
            ...
            "lights" => {
                ...
                if let Some(light_info) = self.daz_knowledge.get_lights_by_type(&format!("{}_light", light_type)).first() {
                    compatibility_notes = format!(
                        "{} light: {}",
                        light_info.r#type, 
                        light_info.typical_use
                    );
                }
        ...
        if lower_name.contains("formal") || lower_name.contains("evening") || lower_name.contains("gown") {
            semantic_tags.push("formal".to_string());
            use_cases.push("Formal occasions".to_string());
        }
        if lower_name.contains("sport") || lower_name.contains("athletic") || lower_name.contains("workout") {
            semantic_tags.push("sport".to_string());
            use_cases.push("Athletic activities".to_string());
        }
        if lower_name.contains("fantasy") || lower_name.contains("medieval") || lower_name.contains("elven") {
            semantic_tags.push("fantasy".to_string());
            use_cases.push("Fantasy settings".to_string());
        }
        if lower_name.contains("sci-fi") || lower_name.contains("futuristic") || lower_name.contains("cyber") {
    ...
    pub fn get_use_cases_for_category(&self, category: &str) -> Vec<String> {
        self.daz_knowledge.asset_categories
            .get(category)
