...
impl DazKnowledgeBase {
    ...
    fn init_poses(&mut self) {
        self.poses.insert(
            ...
            Pose {
                ...
                figure: "Genesis 8 Female".to_string(),
                description: "A neutral standing pose with arms at sides and feet together"
                    .to_string(),
                keyframes: vec![
                    PoseKeyframe {
    ...
    pub fn get_poses_for_figure(&self, figure: &str) -> Vec<&Pose> {
        self.poses.values().filter(|p| p.figure == figure).collect()
    }
