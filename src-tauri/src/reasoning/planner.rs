...
use crate::knowledge::workflow_knowledge::{
    ActionType, WorkflowKnowledgeBase, WorkflowStep, WorkflowType,
};
...
impl Planner {
    ...
    fn select_best_approach(
        &self,
        candidates: Vec<PlanApproach>,
        _context: &PlanningContext,
    ) -> Option<PlanApproach> {
    ...
    fn build_workflow_plan(
        ...
        let workflow =
            workflow_kb.generate_workflow_from_template(workflow_type, &HashMap::new())?;
        ...
        // Calculate totals
        let estimated_time = plan_steps.iter().map(|s| s.estimated_time_seconds).sum();

        let confidence = if plan_steps.is_empty() {
            0.0
        } else {
    ...
    fn build_scene_plan(
        ...
        for lighting_rec in scene_understanding.lighting_setup.iter() {
            plan_steps.push(PlanStep {
                ...
                description: format!(
                    "Add {} light for {} purpose",
                    lighting_rec.light_type, lighting_rec.purpose
                ),
                action: StructuredAiAction {
                ...
                prerequisites: if step_counter > 0 {
                    vec![format!("load_figure_{}", step_counter - 1)]
                } else {
                    Vec::new()
                },
    ...
    fn build_asset_plan(
        ...
        for figure in figures {
            ...
            let figure_type = if figure.to_lowercase().contains('9') {
                "genesis9"
            } else {
                "genesis8"
            };
        ...
        for asset in assets {
            ...
            let (command, args, confidence, sdk_refs) =
                if let Some(path) = resolve_asset_path(&asset, None, context) {
                    (
        ...
        if steps.is_empty() {
            ...
            Some(self.plan_from_steps(
                goal,
                "Asset-focused composition plan",
                steps,
                RiskLevel::Low,
            ))
        }
    ...
    fn workflow_step_to_action(
        ...
        match &step.action_type {
                ...
                match asset_type.as_str() {
                    ...
                    "pose" => {
                            ...
                            .or_else(|| resolve_asset_path(&query, Some("pose"), context))
                        {
                            ...
                            args_json =
                                serde_json::json!({ "pose_path": path, "figure_id": "selected" });
                            conf = 0.82;
                        } else {
                            command_str = "search_content".to_string();
                            args_json = serde_json::json!({
            ...
            ActionType::CreateLight => {
                let light_type_default = "point_light".to_string();
                let light_type = step.parameters.get("type").unwrap_or(&light_type_default);
                Some(StructuredAiAction {
                    command: "add_node".to_string(),
                    args: serde_json::json!({
                        "type": light_type,
            ...
            ActionType::CreateCamera => Some(StructuredAiAction {
                command: "add_node".to_string(),
                args: serde_json::json!({
                    "type": "camera",
                    "name": "AI_Camera"
            ...
            ActionType::Render => Some(StructuredAiAction {
                command: "render_preview".to_string(),
                args: serde_json::json!({}),
                confidence: 0.8,
                sdk_refs: vec!["DzRenderMgr".to_string()],
            ...
            ActionType::ChangeMaterial => Some(StructuredAiAction {
            ...
            ActionType::ExportScene => Some(StructuredAiAction {
            ...
            ActionType::RunSimulation => Some(StructuredAiAction {
            ...
            ActionType::UndoBatchBegin => Some(StructuredAiAction {
            ...
            ActionType::UndoBatchEnd => Some(StructuredAiAction {
            ...
            ActionType::SetMorph => {
                ...
                let morph = step.parameters.get("morph").unwrap_or(&morph_default);
                let value = step.parameters.get("value").unwrap_or(&value_default);
                Some(StructuredAiAction {
            ...
            ActionType::SetExpression => {
                ...
                let expr = step.parameters.get("expression").unwrap_or(&expr_default);
                let value = step.parameters.get("value").unwrap_or(&value_default);
                Some(StructuredAiAction {
            ...
            ActionType::SetCamera => {
                ...
                let camera = step.parameters.get("camera").unwrap_or(&camera_default);
                let focal_length = step.parameters.get("focal_length").unwrap_or(&fl_default);
                let focal_distance = step.parameters.get("focal_distance").unwrap_or(&fd_default);
                Some(StructuredAiAction {
            ...
            ActionType::SetMaterialTexture => {
                ...
                Some(StructuredAiAction {
                    ...
                    confidence: 0.7,
                    sdk_refs: vec![
                        "DzImageProperty".to_string(),
                        "DzDefaultMaterial".to_string(),
                    ],
                    requires_confirmation: false,
                })
            },
    ...
    fn validate_and_refine(&self, plan: Plan, _context: &PlanningContext) -> Option<Plan> {
        ...
        let mut steps: Vec<PlanStep> = plan
            ...
            .filter(|step| {
                ...
                !step
                    ...
                    .and_then(|v| v.as_str())
                    .map(|path| {
                        path.is_empty() || path.contains("TODO") || path.contains("unknown")
                    })
                    .unwrap_or(false)
            })
            .collect();
        ...
        let valid_ids: std::collections::HashSet<String> =
            steps.iter().map(|step| step.id.clone()).collect();
        for step in &mut steps {
            step.prerequisites
                .retain(|prereq| valid_ids.contains(prereq));
        }
        ...
        let estimated_total_time_seconds =
    ...
    fn infer_workflow_type(&self, goal: &Goal) -> Option<WorkflowType> {
        ...
        if lower_desc.contains("scene")
            || lower_desc.contains("full")
            || lower_desc.contains("complete")
        {
            return Some(WorkflowType::CreateScene);
        }
        if lower_desc.contains("create")
            && (lower_desc.contains("character") || lower_desc.contains("figure"))
        {
            return Some(WorkflowType::CreateCharacter);
        }
        if lower_desc.contains("outfit")
            || lower_desc.contains("dress")
            || lower_desc.contains("clothing")
        {
        ...
        if lower_desc.contains("fix")
            || lower_desc.contains("problem")
            || lower_desc.contains("issue")
        {
        ...
        if matches!(goal.intent, Intent::LoadAsset)
            && goal
                .entities
                .iter()
                .any(|e| matches!(e.entity_type, EntityType::Figure))
        {
            return Some(WorkflowType::CreateCharacter);
        }
    ...
    context
        ...
        .find(|asset| {
            let category_matches = category_lower
                .as_ref()
                .map(|category| {
                    asset.category.eq_ignore_ascii_case(category)
                        || asset
                            .category
                            .eq_ignore_ascii_case(category.trim_end_matches('s'))
                })
                .unwrap_or(true);
            category_matches
