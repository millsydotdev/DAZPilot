...
impl Explainer {
    ...
    pub fn explain_plan_selection(&self, plan: &Plan, alternatives: &[Plan], _context: &PlanningContext) -> String {
        ...
        for (i, step) in plan.steps.iter().enumerate() {
            explanation.push_str(&format!("{}. {} ({}s, {:.0}% confidence)\n", 
                i + 1, 
                step.description, 
        ...
        if !alternatives.is_empty() {
            ...
            for (i, alt) in alternatives.iter().enumerate().take(3) { // Show top 3
    ...
    pub fn explain_step(&self, step: &PlanStep, _context: &PlanningContext) -> String {
        ...
        if !step.alternatives.is_empty() {
            explanation.push_str(&format!("Alternatives available: {} options\n", step.alternatives.len()));
        }
    ...
    pub fn explain_failure(&self, _plan: &Plan, failed_step: &PlanStep, error: &str, context: &PlanningContext) -> String {
        let mut explanation = String::new();
        
        explanation.push_str(&format!("The plan failed at step: {}\n", failed_step.description));
...
// Provide suggestions based on failure type
    ...
    pub fn explain_alternative(&self, original: &Plan, alternative: &Plan, _context: &PlanningContext) -> String {
        let mut explanation = String::new();
        
        explanation.push_str(&format!("Alternative plan '{}' might be better because:\n", alternative.description));
        explanation.push_str(&format!("• Higher confidence: {:.0}% vs {:.0}%\n", 
            alternative.confidence * 100.0, 
            original.confidence * 100.0));
        explanation.push_str(&format!("• Lower estimated time: {}s vs {}s\n", 
            alternative.estimated_total_time_seconds,
            original.estimated_total_time_seconds));
        explanation.push_str(&format!("• Lower risk: {:?} vs {:?}\n", 
    ...
    fn describe_command_effect(&self, command: &str, args: &serde_json::Value) -> String {
        match command {
            "add_figure" => {
                let figure_type = args.get("figure_type").and_then(|v| v.as_str()).unwrap_or("unknown");
                let figure_name = if figure_type == "genesis9" { "Genesis 9 Female" } else { "Genesis 8 Female" };
            ...
            "apply_pose" => {
                let pose = args.get("pose").and_then(|v| v.as_str()).unwrap_or("unknown");
                format!("Apply the {} pose", pose)
            }
            "set_keyframe" => {
                let node_id = args.get("node_id").and_then(|v| v.as_str()).unwrap_or("unknown");
                let property = args.get("property").and_then(|v| v.as_str()).unwrap_or("unknown");
                let frame = args.get("frame").and_then(|v| v.as_i64()).unwrap_or(0);
                let value = args.get("value").and_then(|v| v.as_f64()).unwrap_or(0.0);
                format!("Set {} on {} to {} at frame {}", property, node_id, value, frame)
            }
            "add_node" => {
                let node_type = args.get("type").and_then(|v| v.as_str()).unwrap_or("unknown");
            ...
            "set_property" => {
                let node_id = args.get("node_id").and_then(|v| v.as_str()).unwrap_or("unknown");
                let property = args.get("property").and_then(|v| v.as_str()).unwrap_or("unknown");
                let value = args.get("value").and_then(|v| v.as_str()).unwrap_or("unknown");
            ...
            "render_preview" => {
                "Generate a preview render of the current scene".to_string()
            }
            "export_scene" => {
                let format = args.get("settings").and_then(|s| s.get("format").and_then(|v| v.as_str())).unwrap_or("unknown");
                format!("Export the scene as a {} file", format)
            }
            "run_dforce_simulation" => {
                let node_id = args.get("node_id").and_then(|v| v.as_str()).unwrap_or("unknown");
                let start = args.get("start_frame").and_then(|v| v.as_i64()).unwrap_or(0);
                let end = args.get("end_frame").and_then(|v| v.as_i64()).unwrap_or(30);
                format!("Run a dForce physics simulation on {} from frame {} to {}", node_id, start, end)
            }
            "begin_undo_batch" => {
                "Start recording changes for undo/redo".to_string()
            }
            "accept_undo_batch" => {
                "Accept and name the recorded changes".to_string()
            }
            "cancel_undo_batch" => {
                "Discard the recorded changes".to_string()
            }
            _ => {
    ...
    fn generate_failure_suggestions(&self, step: &PlanStep, error: &str, _context: &PlanningContext) -> Vec<String> {
        ...
        if error.contains("not found") || error.contains("failed to find") {
            ...
            suggestions.push("Make sure the asset is installed and indexed in your library".to_string());
        }
        ...
        if error.contains("invalid") || error.contains("unsupported") {
            ...
            suggestions.push("Check if the asset type is compatible with the operation".to_string());
        }
        ...
        if step.action.command == "add_figure" {
            ...
            suggestions.push("Try checking the exact figure name in your content library".to_string());
        }
        
        if step.action.command.starts_with("set_") && error.contains("property") {
            suggestions.push("Verify that the property name is correct for the selected node type".to_string());
            suggestions.push("Try checking what properties are available for this node type".to_string());
        }
    ...
    pub fn explain_plan_outcome(&self, plan: &Plan) -> String {
        ...
        if figures_added > 0 {
            explanation.push_str(&format!("• Add {} figure{}\n", 
                figures_added, 
                if figures_added == 1 { "" } else { "s" }));
        }
        if assets_loaded > 0 {
            explanation.push_str(&format!("• Load {} asset{}\n", 
                assets_loaded, 
                if assets_loaded == 1 { "" } else { "s" }));
        }
        if lights_added > 0 {
            explanation.push_str(&format!("• Add {} light{}\n", 
                lights_added, 
                if lights_added == 1 { "" } else { "s" }));
        }
        if poses_applied > 0 {
            explanation.push_str(&format!("• Apply {} pose{}\n", 
                poses_applied, 
                if poses_applied == 1 { "" } else { "s" }));
        }
        if renders_done > 0 {
            explanation.push_str(&format!("• Generate {} render{}\n", 
                renders_done, 
