...
pub async fn analyze_current_viewport() -> Result<SceneAnalysis, String> {
    ...
    let image_bytes =
        std::fs::read(&capture_path).map_err(|e| format!("Failed to read capture: {}", e))?;
...
pub fn fetch_spatial_context() -> String {
    ...
    let response = match result {
        Ok(res) => res,
        Err(_) => {
            return "Spatial Awareness Error: Daz Studio is disconnected or not responding."
                .to_string()
        },
    };
...
pub fn detect_asset_conflicts_from_scene() -> AssetConflictReport {
    ...
    let loaded_assets: Vec<String> =
        match mcp_client::send_mcp_request("get_scene_assets", serde_json::json!({})) {
...
mod tests {
    ...
    fn test_spatial_relationships() {
        ...
        assert!(
            relations.contains("'Genesis 8 Female' is to the left of and behind and above 'Chair'")
        );
        assert!(relations
            .contains("'Chair' is to the right of and in front of and below 'Genesis 8 Female'"));
    }
}
