...
fn test_create_scene_workflow_detected() {
    ...
    let goal = make_goal("Build a complete scene from scratch", Intent::CreateScene, vec![]);
    let plan = planner.plan_for_goal(&goal, &empty_context());
    assert!(plan.is_some(), "Planner should return a plan for scene creation");
    let plan = plan.unwrap();
    assert!(
        plan.description.contains("Scene") || plan.steps.len() >= 10,
        "Plan description should reference scene or have 10+ steps, got: {} ({} steps)", plan.description, plan.steps.len()
    );
    assert!(!plan.steps.is_empty(), "Scene workflow should have steps");
    assert_eq!(plan.steps[0].action.command, "begin_undo_batch", "First step should begin undo batch");
}

#[test]
fn test_create_scene_workflow_full_steps() {
    let planner = Planner::new();
    let goal = make_goal("Create a full scene from scratch", Intent::CreateScene, vec![]);
    let plan = planner.plan_for_goal(&goal, &empty_context());
    assert!(plan.is_some());
    let plan = plan.unwrap();

    let commands: Vec<&str> = plan.steps.iter().map(|s| s.action.command.as_str()).collect();
    assert!(commands.contains(&"add_figure"), "Should add a figure");
    assert!(commands.contains(&"add_node"), "Should create lights via add_node");
...
fn test_create_character_workflow() {
    let planner = Planner::new();
    let goal = make_goal("create a genesis figure with clothing", Intent::LoadAsset,
        vec![Entity { entity_type: EntityType::Figure, value: "genesis9".into(), confidence: 0.9 }]);
    let plan = planner.plan_for_goal(&goal, &empty_context());
    assert!(plan.is_some(), "Should create character workflow");
    let plan = plan.unwrap();
    let cmds: Vec<&str> = plan.steps.iter().map(|s| s.action.command.as_str()).collect();
    assert_eq!(cmds[0], "add_figure", "Character workflow starts with loading the figure");
}
...
fn test_setup_lighting_workflow() {
    ...
    let plan = plan.unwrap();
    let light_steps: Vec<&PlanStep> = plan.steps.iter().filter(|s| s.action.command == "add_node").collect();
    assert!(light_steps.len() >= 3, "Should create at least 3 lights (key/fill/rim), got {}", light_steps.len());
}
...
fn test_pose_character_workflow() {
    ...
    let plan = plan.unwrap();
    let pose_commands: Vec<&str> = plan.steps.iter().map(|s| s.action.command.as_str()).collect();
...
fn test_render_still_workflow() {
    ...
    let plan = plan.unwrap();
    let cmds: Vec<&str> = plan.steps.iter().map(|s| s.action.command.as_str()).collect();
    assert!(cmds.contains(&"render_preview"), "Should call render_preview");
    assert!(cmds.contains(&"begin_undo_batch"), "Should wrap in undo batch");
}
...
fn test_animate_character_workflow() {
    ...
    let plan = plan.unwrap();
    let cmds: Vec<&str> = plan.steps.iter().map(|s| s.action.command.as_str()).collect();
    assert!(cmds.contains(&"set_timeline_range"), "Should set timeline range");
}
...
fn test_workflow_step_confidence() {
    ...
    for step in &plan.steps {
        assert!(step.confidence > 0.0, "Step '{}' should have confidence > 0", step.description);
        assert!(step.estimated_time_seconds > 0, "Step '{}' should have estimated time > 0", step.description);
    }
}
...
fn test_goal_without_known_keywords_returns_plan() {
    ...
    let plan = planner.plan_for_goal(&goal, &empty_context());
    assert!(plan.is_none() || !plan.as_ref().unwrap().steps.is_empty(),
...
fn test_action_to_command_map_covers_all_action_types() {
    ...
    let all_types: HashSet<ActionType> = vec![
        ActionType::LoadAsset, ActionType::ApplyPose, ActionType::AdjustProperty,
        ActionType::CreateLight, ActionType::CreateCamera, ActionType::Render,
        ActionType::ChangeMaterial, ActionType::ExportScene, ActionType::RunSimulation,
        ActionType::UndoBatchBegin, ActionType::UndoBatchEnd, ActionType::AddFigure,
        ActionType::SetMorph, ActionType::SetExpression, ActionType::SetCamera,
        ActionType::SetRenderOptions, ActionType::SetMaterialTexture, ActionType::Animate,
        ActionType::RunScript, ActionType::SearchContent, ActionType::ListBones,
        ActionType::SetBoneTransform,
    ].into_iter().collect();

    let missing: Vec<&ActionType> = all_types.difference(&mapped).collect();
    assert!(missing.is_empty(), "Action types missing from command map: {:?}", missing);
}
...
fn test_command_knowledge_base_has_all_commands() {
    ...
    for schema in &schemas {
        ...
        assert!(cmd.is_some(), "Command '{}' present in MCP schemas but missing from command knowledge base", schema.name);
    }
}
...
fn test_command_knowledge_parameter_accuracy() {
    ...
    for schema in &schemas {
        ...
        let expected_params: std::collections::BTreeSet<&str> = schema.parameters.iter().map(|s| *s).collect();
        let actual_params: std::collections::BTreeSet<&str> = cmd.parameters.iter().map(|s| *s).collect();
...
fn test_high_risk_consistency() {
    ...
    for schema in &schemas {
        ...
        assert_eq!(cmd.high_risk, schema.high_risk,
...
fn test_scene_workflow_commands_exist() {
    ...
    assert!(names.contains(&"render_preview"), "Should include render_preview");
}
...
fn test_find_commands_for_purpose() {
    ...
    assert!(names.contains(&"add_node"), "Should find add_node (for creating lights)");
}
...
fn test_build_command_catalog_prompt_succeeds() {
    ...
    assert!(prompt.contains("Available Daz Bridge commands:"), "Prompt should have header");
    assert!(prompt.contains("add_figure"), "Prompt should include add_figure");
    assert!(prompt.contains("set_morph"), "Prompt should include set_morph");
    assert!(prompt.contains("render_preview"), "Prompt should include render_preview");
    assert!(prompt.contains("[HIGH RISK]"), "Prompt should mark high-risk commands");
}
