...
fn test_simulation_quality_substeps() {
    ...
    assert_eq!(
        ultra.data.unwrap().get("substeps").and_then(|v| v.as_i64()),
        Some(10)
    );
}
...
fn test_frame_calculations() {
    ...
    assert_eq!(
        sim.frames_computed, 40,
        "Simulation computed frames should equal difference."
    );

    let bake = physics::bake_to_keyframes(0, 100);
    assert_eq!(
        bake.keyframe_count, 100,
        "Baked keyframes count must equal frames difference."
    );
...
fn test_collision_zone_addition() {
    ...
    assert!(
        res.message.contains("test_shield"),
        "Message should echo the custom zone name."
    );
}
