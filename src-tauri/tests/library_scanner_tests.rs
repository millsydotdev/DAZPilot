...
fn test_recursive_library_scanner_pipeline() {
    ...
    // 5. Verify results
    assert_eq!(result.total_files, 2, "Scanner should discover exactly 2 supported files.");
    
    // Assert figures categorization
    assert!(!result.categorized.figures.is_empty(), "Genesis 8 Female must be categorized as a figure.");
    ...
    assert!(!result.categorized.clothing.is_empty(), "Sleek Shirt must be categorized as clothing.");
    let shirt_asset = &result.categorized.clothing[0];
    assert_eq!(shirt_asset.name, "Sleek Shirt");
    assert_eq!(shirt_asset.metadata.get("vendor").map(|s| s.as_str()), Some("VendorA"));
}
