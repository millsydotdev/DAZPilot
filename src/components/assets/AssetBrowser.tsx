...
import { Button, Input, VStack, HStack, Text, PanelShell } from '../ui';
...
export default function AssetBrowser() {
  ...
  const fetchRecommendations = useCallback(async () => {
    const theme = search.trim() || filter;
    if (!theme || theme === 'all') return;
    setLoadingRecommendations(true);
    try {
      const result = await invoke<{
        suggestions: Array<{ name: string; reason: string; category: string }>;
      ...
      console.error('Asset recommendations failed:', err);
      setRecommendations([]);
    } finally {
        ...
        <div className={styles.header}>
          ...
          <HStack gap="xs" className={styles.controls}>
            ...
            <Button
              ...
              disabled={loadingRecommendations || (!search.trim() && filter === 'all')}
              icon={
                        ...
                        <VStack gap="xs">
                          ...
                          <HStack>
                            <Input
                              ...
                              value={
                                advancedFilters.dateRange?.start?.toString().split('T')[0] || ''
                              }
                              onChange={(e) => {
